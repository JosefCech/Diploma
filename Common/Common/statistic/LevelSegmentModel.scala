...
trait LevelSegmentModel extends LevelModel {
 ...
 var lamba1 : Double = 1 - lamba0
  def learnProbabilities : List[((Int,String),(Int,Int))] = {
    var double = this.getSimpleTags.flatten.map(t => (t.Level,t.simpleTag)).groupBy(t => t).map(f => (f._1, f._2.length))
  ...
  val getProbMax : Double = this.probabilities.map(f => this.getProbability(f._1._1, f._1._2)).max
}